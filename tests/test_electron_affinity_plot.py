import pytest
from pages.plot_electron_affinity import PubChem_Periodic_Table_Electron_Affinity
import sys, os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

screenshots_loc = os.path.join(os.path.dirname(__file__), "..", "screenshots", "Electron_affinity_plot")
os.makedirs(screenshots_loc, exist_ok=True)

@pytest.mark.usefixtures("setup")
class Test_element_electron_affinity:     
    def test_BY_value_attribute(self):
        page = PubChem_Periodic_Table_Electron_Affinity(self.driver)
