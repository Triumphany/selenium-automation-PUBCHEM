import pytest
from pages.atomic_radius_charts import PubChem_Periodic_Table_Atomic_Radius
import sys, os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

screenshots_loc = os.path.join(os.path.dirname(__file__), "..", "screenshots", "AtomicRADIUS_chart")
os.makedirs(screenshots_loc, exist_ok=True)

@pytest.mark.usefixtures("setup")
class Test_element_atomic_masses_selection:     
    def test_BY_value_attribute(self):
        page = PubChem_Periodic_Table_Atomic_Radius(self.driver)
