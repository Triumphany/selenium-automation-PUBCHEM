import pytest
from pages.electronagativity_charts import PubChem_Periodic_Table_Electronagativity
import sys, os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

screenshots_loc = os.path.join(os.path.dirname(__file__), "..", "screenshots", "Electronegativity_chart")
os.makedirs(screenshots_loc, exist_ok=True)

@pytest.mark.usefixtures("setup")
class Test_element_atomic_masses_selection:     
    def test_BY_value_attribute(self):
        page = PubChem_Periodic_Table_Electronagativity(self.driver)
